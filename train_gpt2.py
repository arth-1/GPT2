...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.gelu= nn.GELU(approximate='tanh') #try using normal appriximate = none too
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        #weighgt sharing scheme idk..it improves performance ig
...
T=1024 
assert total_batch_size % (B*T) == 0, "total_batch_size should be divisible by B*T"
grad_accum_steps = total_batch_size // (B*T)
print(f"total desired batch size: {total_batch_size}")
print(f"=> calculated gradient accumulation steps: {grad_accum_steps}")

train_loder = DataLoaderLite(B=B, T=T, split="train") 
...
for step in range(max_steps):
    ...
    if step % 250 == 0 or last_step:
        ...
        if step >0 and (step % 5000 == 0 or last_step):
            ...
            #I would need to add optimizer.state_dict() and 
            # rng seeds etc. so that i can exactly resume training
    ...
    t1 = time.time()
    dt = (t1-t0)
    tokens_processed = train_loder.B * train_loder.T * grad_accum_steps
...
while x.size(1)<max_length:
    #forward the mdoel to get logits
    with torch.no_grad():
        logits = model(x)[0]  # Extract the first element
        logits = logits[:, -1, :]

        #probablity
        probs = F.softmax(logits, dim=-1)
        #Top k sampling idk why 50 but ig its huggingface default
        #top-k probs here become (5,50 ), top k indices is (5,50)
        topk_probs, topk_indices = torch.topk(probs, 50, dim=-1)
        #select token from topk probs
        ix = torch.multinomial(topk_probs,1) #(B,1)
        #gather corresponding indidces
        xcol = torch.gather(topk_indices, -1, ix) #(B,1)
        #append to sequence
